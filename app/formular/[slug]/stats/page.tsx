...
import { formPageStorage, formSubmissionStorage, analyticsStorage } from '@/lib/storage';
import { funnelStorage } from '@/lib/funnel-storage';
import { FunnelDoc } from '@/lib/funnel-types';
import { FormPage, FormSubmission, AnalyticsEvent } from '@/lib/types';
...
export default function FormStatsPage() {
  ...
  const [form, setForm] = useState<FormPage | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      setForm(found);
      const [subs, evs, fd] = await Promise.all([
        formSubmissionStorage.getByForm(id),
        analyticsStorage.getByForm(id),
  ...
  const pageStats = (() => {
    ...
    for (const s of bySession.values()) {
      for (const p of s.pages) v.set(p, (v.get(p) ?? 0) + 1);
      if (!s.completed && s.lastPage) x.set(s.lastPage, (x.get(s.lastPage) ?? 0) + 1);
    }
    return funnelDoc.pages.map((p) => {
      const vw = v.get(p.id) ?? 0;
      const xv = x.get(p.id) ?? 0;
      return { pageId: p.id, views: vw, exits: xv, exitRate: vw > 0 ? Math.round((xv / vw) * 100) : 0 };
    });
  })();
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-4 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="font-semibold text-slate-900">{form.title}</h1>
            <p className="text-xs text-slate-500">Statistiken & Einreichungen</p>
          </div>
          {form.status === 'published' && (
            <Link href={`/formular/${form.slug}`} target="_blank" className="btn-secondary text-xs py-1.5">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {[
            ...
            { label: 'Einreichungen', value: filteredSubs.length, icon: Users, color: 'rose' },
        ...
        <div className="card p-5">
          ...
          <div className="space-y-4">
            {[
              ...
              { label: 'Einreichungen', value: filteredSubs.length, max: Math.max(views, 1), color: 'bg-amber-400' },
            ...
            {pageStats.every((p) => p.views === 0) ? (
              ...
              <div className="overflow-x-auto">
                ...
                <p className="px-5 py-3 text-xs text-slate-400 border-t border-slate-100">
                  Absprungrate = Sessions, die auf dieser Seite verlassen haben, ohne das Formular abzuschließen.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="card p-5">
          <h2 className="font-semibold text-slate-900 mb-3">Formular-Informationen</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              <p className="text-slate-500 text-xs mb-0.5">Inhaltsblöcke</p>
              <p className="font-medium text-slate-900">{form.contentBlocks.length}</p>
            </div>
            <div>
              <p className="text-slate-500 text-xs mb-0.5">Formular-Schritte</p>
              <p className="font-medium text-slate-900">{form.formSteps.length}</p>
            </div>
            <div>
              <p className="text-slate-500 text-xs mb-0.5">Erstellt am</p>
              <p className="font-medium text-slate-900">{formatDateShort(form.createdAt)}</p>
            </div>
            {form.publishedAt && (
              <div>
                <p className="text-slate-500 text-xs mb-0.5">Veröffentlicht am</p>
                <p className="font-medium text-slate-900">{formatDateShort(form.publishedAt)}</p>
              </div>
            )}
            {form.status === 'published' && (
              <div className="col-span-2">
                <p className="text-slate-500 text-xs mb-0.5">Öffentliche URL</p>
                <p className="font-mono text-xs text-violet-600 break-all">/formular/{form.slug}</p>
              </div>
            )}
          </div>
        </div>
